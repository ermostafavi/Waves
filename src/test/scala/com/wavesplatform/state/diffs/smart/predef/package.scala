...
package object predef {
  ...
  def scriptWithAllFunctions(tx: DataTransaction, t: TransferTransaction): String =
    s"""
       ...
       |   case d: DataTransaction =>
       |     let int = extract(getInteger(d.data, "int"))
       |     let bool = extract(getBoolean(d.data, "bool"))
       |     let blob = extract(getBinary(d.data, "blob"))
       |     let str = extract(getString(d.data, "str"))
       |     let dataByKey = toString(int) == "24" || toString(bool) == "true" || size(blob) > 0 || str == "str"
       |
       |     let d0 = extract(getInteger(d.data, 0))
       |     let d1 = extract(getBoolean(d.data, 1))
       |     let d2 = getBinary(d.data, 2)
       |     let d3 = getString(d.data, 3)
       |     let dataByIndex = toBytes(d0) == base64'abcdef' && toBytes(d1) == base64'ghijkl' && isDefined(d2) && toBytes(extract(d3)) == base64'mnopqr'
       |
